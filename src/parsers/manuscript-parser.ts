...
export class ManuscriptParser {
  ...
  private parseChapters(content: string): Chapter[] {
    ...
    const chapterHeadingRegex =
      /^(?:#{1,2}\s+(?:Capítulo|Chapter|Cap\.?)\s*\d+|#{1,2}\s+\d+\s*[-–—:.]?\s*.+|(?:Capítulo|Chapter|Cap\.?)\s*\d+|#{1,2}\s+.+)$/gim;
  ...
  getSamples(content: string, sampleSize: number = 500, sampleCount: number = 10): string[] {
    ...
    if (totalWords <= sampleSize * sampleCount) {
      ...
      return this.splitIntoChunks(content, Math.ceil(totalWords / sampleCount)).map((chunk) =>
        chunk.slice(0, sampleSize),
      );
    }
