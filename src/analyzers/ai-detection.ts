...
const ARTIFACT_MARKERS = {
  ...
  E1: {
    name: 'Americanismos em contexto não-americano',
...
export class AIDetectionAnalyzer {
    ...
    if (ttr < 0.12 || ttr > 0.35) {
      artifacts.push(
        this.createArtifact('D3', [
          {
            location: 'Global',
            excerpt: `TTR: ${ttr.toFixed(3)} (${
              ttr < 0.12 ? 'vocabulário repetitivo' : 'vocabulário atipicamente diverso'
            })`,
            chapterIndex: 0,
          },
        ]),
  ...
  private analyzeLexicalPatterns(content: string): AIArtifact[] {
    ...
    if (intensifierCount / wordCount > 0.005) {
      // More than 0.5% is suspicious
      artifacts.push(
        this.createArtifact('A4', [
          {
            location: 'Global',
            excerpt: `${intensifierCount} intensificadores em ${wordCount} palavras (${(
              (intensifierCount / wordCount) *
              100
            ).toFixed(2)}%)`,
            chapterIndex: 0,
          },
    ...
    if (coefficientOfVariation < 0.3) {
      artifacts.push(
        this.createArtifact('B1', [
          {
            location: 'Global',
            excerpt: `Coeficiente de variação: ${coefficientOfVariation.toFixed(
              3,
...
Seja objetivo e específico. Não faça julgamentos morais.`;
  ...
  private calculateIUL(artifacts: AIArtifact[], samples: SampleAnalysis[]): number {
    ...
    const avgSampleIUL =
      samples.length > 0 ? samples.reduce((sum, s) => sum + s.localIul, 0) / samples.length : 0;
